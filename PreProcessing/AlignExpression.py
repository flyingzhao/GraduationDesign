...
for root, dirs, files in os.walk("./Expression", topdown=False):
    for name in files:
        all_img.append(os.path.join(root, name))

for name in all_img:
    pname=name.split("\\")[-2]
    if not os.path.exists("./Align_expression/"+pname):
        os.makedirs("./Align_expression/"+pname)
    bgr=cv2.imread(name)
    rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
    bb, landmark, H, outRgb = align.align(256, rgb, face_cascade)

    outBgr = cv2.cvtColor(outRgb, cv2.COLOR_RGB2BGR)
    cv2.imwrite("./Align_expression/"+pname+"/"+name.split("\\")[-1],outBgr)
    print(name)
